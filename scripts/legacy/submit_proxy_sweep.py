"""
Submit the proxy validation sweep to Scheduler.

Submits 5 chained jobs (P1-P5) via the Scheduler REST API.
Each job depends on the previous one completing, so they run
sequentially on gpu-host's 8×B200 GPUs.
...
def submit_sweep(
    ...
    if httpx is not None:
        ...
        client = _FallbackClient(SCHEDULER_URL)

    # Verify Scheduler is reachable
    try:
        resp = client.get("/api/v1/status")
        if hasattr(resp, 'status_code') and resp.status_code != 200:
            print(f"ERROR: Scheduler returned status {resp.status_code}")
            sys.exit(1)
    except Exception as e:
        print(f"ERROR: Cannot reach Scheduler at {SCHEDULER_URL}: {e}")
        sys.exit(1)
...
def check_status() -> None:
    ...
    try:
        ...
        if not our_jobs:
            print("No luxia-base jobs found in Scheduler")
            return
...
def main() -> None:
    p = argparse.ArgumentParser(description="Submit proxy sweep to Scheduler")
