#!/usr/bin/env python3
"""Extract benchmark results from Scheduler job logs.
...
def get_job(job_id: str) -> dict:
    result = subprocess.run(
        ["curl", "-s", f"{SCHEDULER_URL}/api/v1/jobs/{job_id}"],
...
def list_jobs(prefix: str, limit: int = 20) -> list[dict]:
    result = subprocess.run(
        ["curl", "-s", f"{SCHEDULER_URL}/api/v1/jobs?limit={limit}"],
        capture_output=True, text=True, timeout=10,
    )
...
def extract_metrics(log_tail: str) -> dict:
    """Extract tok/s, mem, loss from Scheduler log tail."""
...
def get_full_log(job_id: str) -> str:
    """Get full log from Scheduler."""
    result = subprocess.run(
        ["curl", "-s", f"{SCHEDULER_URL}/api/v1/jobs/{job_id}/logs?lines=500"],
        capture_output=True, text=True, timeout=10,
    )
    try:
        data = json.loads(result.stdout)
