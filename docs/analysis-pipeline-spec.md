...
```bash
# 1. Launch serve.py on a GPU node (via Scheduler or directly)
scheduler submit 'tools/run_eval.sh serve.py --checkpoint <path> --device cuda --port 2222 --compile' \
    --name eval-serve --type custom --gpus 1 --node gpu-host \
    --workdir /home/cluster-user/workspace/kotodama
...
```bash
# Via Scheduler
scheduler submit 'tools/run_eval.sh scripts/analysis/eval_generate.py \
    --checkpoint <path> --name <run-name> \
    --attn-res "boundaries=0,3,7,12,21,25" --no-ppl \
    --prompt-set extended --n-samples 3' \
    --name eval-gen --type custom --gpus 1 --node gpu-host \
    --workdir /home/cluster-user/workspace/kotodama
```
...
| `eval_proxy_sweep.py` | Track 3 | Keeps Scheduler logging pattern |
...
- [ ] Full GPU validation against proxy run outputs (deferred — needs gpu-host)
...
- **Scheduler integration:** Optional `--scheduler-job-id` flag in Track 3 if desired, but not a priority. Separate concern from the core pipeline.
