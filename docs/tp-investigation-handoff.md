...
All code is at `~/projects/kotodama/pretraining/` (local) and `~/workspace/kotodama/` (gpu-host).
...
### Throughput validation (gpu-host, 8xB200)
...
- **gpu-host**: `ssh gpu-host`, user `cluster-user`, working dir `~/workspace/kotodama/`
- **Shared venv**: `~/workspace/.venv-shared/` — use `uv` for package management, NEVER `pip`
